...
#### Custom filesystem paths (e.g. NFS)

For anonymous results and shuffle data, you can point Polars at a custom filesystem path instead of using S3 or ephemeral volumes. This is intended for shared network filesystems such as NFS or CephFS, where all pods can access the same data through a common mount point. First, add the NFS volume and mount it into the container via the `volumes` and `volumeMounts` values, then set the `customPath` to tell Polars to use that mount point.

> **Note:** Custom filesystem paths are designed for shared network filesystems. For shuffle data in particular, all workers must be able to read and write to the same path — the `shared_filesystem` shuffle location is used under the hood.
...
| anonymousResults.customPath | object | `{"enabled":false,"path":"/mnt/nfs/anonymous-results"}` | Configure a custom filesystem path for anonymous results (e.g. an NFS mount). You must mount the volume yourself using scheduler.deployment.volumes and scheduler.deployment.runtimeContainer.volumeMounts. |
| anonymousResults.customPath.enabled | bool | `false` | Enable custom path for anonymous results. |
| anonymousResults.customPath.path | string | `"/mnt/nfs/anonymous-results"` | The filesystem path where anonymous results will be stored. Must correspond to a mounted volume. |
...
| shuffleData.customPath | object | `{"enabled":false,"path":"/mnt/nfs/shuffle-data"}` | Configure a custom filesystem path for shuffle data (e.g. an NFS mount). Intended for shared network filesystems where all workers access the same path. You must mount the volume yourself using worker.deployment.volumes and worker.deployment.runtimeContainer.volumeMounts. |
| shuffleData.customPath.enabled | bool | `false` | Enable custom path for shuffle data. |
| shuffleData.customPath.path | string | `"/mnt/nfs/shuffle-data"` | The filesystem path where shuffle data will be stored. Must correspond to a mounted volume on a shared network filesystem. |
